...
The `home-routes` handler in the `rocket-link.routes.home` namespace
...
The `home-page` function will in turn call the `rocket-link.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `rocket-link.handler` namespace:
...
Request middleware functions are located under the `rocket-link.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `rocket-link.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `rocket-link.db.core` in some other namespace.
